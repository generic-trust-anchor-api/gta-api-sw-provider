...
int pkey_ec_nid(const EVP_PKEY * evp_private_key)
{
    ...
    if (!EVP_PKEY_get_utf8_string_param(evp_private_key, OSSL_PKEY_PARAM_GROUP_NAME, curve_name, sizeof(curve_name),
                                        &len)) {
        return 0;
    }
...
bool read_input_buffer(gtaio_istream_t * data, unsigned char ** pp_data, size_t * p_data_size,
