                         ...
                         struct personality_attribute_t ** p_pers_attribute, gta_errinfo_t * p_errinfo))
{
    ...
    *p_pers_secret_type = SECRET_TYPE_DILITHIUM2;
    ...
    if (!add_personality_attribute_list_item(
            p_provider_params, p_pers_attribute, PAT_COM_GITHUB_GENERIC_TRUST_ANCHOR_API_KEYTYPE_OPENSSL,
            (unsigned char *)PERS_ATTR_NAME_KEYTYPE, sizeof(PERS_ATTR_NAME_KEYTYPE),
            (unsigned char *)PERS_ATTR_KEYTYPE_DILITHIUM2, sizeof(PERS_ATTR_KEYTYPE_DILITHIUM2), true, p_errinfo)) {
