        ...
        IMPLEMENT_ASN1_FUNCTIONS(ProtectedData)
            ...
            GTA_SWP_DEFINE_FUNCTION(bool, seal_data,
                                    (struct gta_sw_provider_context_params_t * p_context_params, gtaio_istream_t * data,
                                     gtaio_ostream_t * protected_data, gta_errinfo_t * p_errinfo))
{
    bool ret = false;
    ...
    key = gta_secmem_calloc(p_context_params->h_ctx, (size_t)EVP_CIPHER_get_key_length(EVP_aes_256_gcm()),
                            sizeof(unsigned char), p_errinfo);
    if (NULL == key) {
        *p_errinfo = GTA_ERROR_INTERNAL_ERROR;
        goto err;
    }
    ...
    HMAC(EVP_sha256(), p_personality_content->secret_data, (int)p_personality_content->secret_data_size, key_derivation,
...
GTA_SWP_DEFINE_FUNCTION(bool, unseal_data,
                        (struct gta_sw_provider_context_params_t * p_context_params, gtaio_istream_t * protected_data,
                         gtaio_ostream_t * data, gta_errinfo_t * p_errinfo))
{
    bool ret = false;
    gta_errinfo_t errinfo_tmp;
    const struct personality_t * p_personality_content = NULL;
    ...
    /* Derive a key from the personality secret */
    HMAC(EVP_sha256(), p_personality_content->secret_data, (int)p_personality_content->secret_data_size,
         p_data->key->data, LOCAL_DATA_PROTECTION_KEY_DERIVATION_LEN, key, &size);
