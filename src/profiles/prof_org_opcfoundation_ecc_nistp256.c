...
GTA_SWP_DEFINE_FUNCTION(bool, personality_create,
                        (struct gta_sw_provider_params_t * p_provider_params, gta_personality_name_t personality_name,
                         personality_secret_type_t * p_pers_secret_type, unsigned char ** p_pers_secret_buffer,
                         size_t * p_pers_secret_length, gta_personality_fingerprint_t pers_fingerprint,
                         struct personality_attribute_t ** p_pers_attribute, gta_errinfo_t * p_errinfo))
{
    EVP_PKEY * p_key = NULL;
                        ...
                        (struct gta_sw_provider_context_params_t * p_context_params, gta_errinfo_t * p_errinfo))
{
    ...
    evp_private_key = d2i_AutoPrivateKey(NULL, (const unsigned char **)&p_secret_buffer,
...
GTA_SWP_DEFINE_FUNCTION(bool, context_get_attribute,
                        (struct gta_sw_provider_context_params_t * p_context_params,
                         gta_context_attribute_type_t attrtype, gtaio_ostream_t * p_attrvalue,
...
GTA_SWP_DEFINE_FUNCTION(bool, context_set_attribute,
                        (struct gta_sw_provider_context_params_t * p_context_params,
                         gta_context_attribute_type_t attrtype, gtaio_istream_t * p_attrvalue,
                         gta_errinfo_t * p_errinfo))
{
    struct pers_enroll_attributes_t * pers_enroll_attributes =
        (struct pers_enroll_attributes_t *)p_context_params->context_attributes;
        ...
        if (NULL == pers_enroll_attributes->x509_name) {
            *p_errinfo = GTA_ERROR_INTERNAL_ERROR;
            return false;
               ...
               (NULL == pers_enroll_attributes->san_names)) {
                         ...
                         gtaio_ostream_t * p_personality_enrollment_info, gta_errinfo_t * p_errinfo))
{
    ...
    unsigned char * p_secret_buffer = p_personality_content->secret_data;
    p_key = d2i_AutoPrivateKey(NULL, (const unsigned char **)&p_secret_buffer,
                               (long)p_personality_content->secret_data_size);
    ...
    if ((1 != X509_REQ_set_pubkey(x509_req, p_key)) || (0 >= X509_REQ_sign(x509_req, p_key, EVP_sha256()))) {
        goto internal_err;
    }
    ...
    size_t buffer_idx_out = (size_t)len;
    if ((len < 0) ||
        (buffer_idx_out != p_personality_enrollment_info->write(p_personality_enrollment_info, (char *)p_buffer_out,
                                                                buffer_idx_out, p_errinfo))) {
        goto internal_err;
    }
...
GTA_SWP_DEFINE_FUNCTION(bool, authenticate_data_detached,
                        (struct gta_sw_provider_context_params_t * p_context_params, gtaio_istream_t * data,
                         gtaio_ostream_t * seal, gta_errinfo_t * p_errinfo))
{
    bool ret = false;
    ...
    if ((!(mdctx = EVP_MD_CTX_new())) || (1 != EVP_DigestSignInit(mdctx, NULL, EVP_sha256(), NULL, evp_private_key))) {
        goto internal_err;
    }
