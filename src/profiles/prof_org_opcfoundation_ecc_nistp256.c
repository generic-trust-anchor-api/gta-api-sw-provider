     ...
     gta_errinfo_t * p_errinfo))
{
    ...
    if (NULL != pers_enroll_attributes->san_names) {
        ...
        if (0 != strcmp(
                     ...
                     p_context_params->p_personality_item->p_identifier_list_item->type)) {
            *p_errinfo = GTA_ERROR_ITEM_NOT_FOUND;
            goto cleanup;
        }
        ...
        if ((NULL == san_names) || (NULL == gen_name) || (NULL == ia5)) {
            goto internal_err;
        }
    ...
    size_t buffer_idx_out = (size_t)len;
    if ((len < 0) ||
        (buffer_idx_out != p_personality_enrollment_info->write(
                               p_personality_enrollment_info, (char *)p_buffer_out, buffer_idx_out, p_errinfo))) {
        goto internal_err;
    }
