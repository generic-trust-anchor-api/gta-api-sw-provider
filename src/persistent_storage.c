                                     ...
                                     const struct auth_info_list_item_t * p_auth_list)
{
    ...
    while (NULL != p_auth_item) {
        ...
        if (GTA_ACCESS_DESCRIPTOR_TYPE_PERS_DERIVED_TOKEN == p_auth_item->type) {
            ...
            QCBOREncode_AddSZStringToMap(p_encode_ctx, LABEL_AUTH_DERIVATION_PROFILE_NAME,
                                         p_auth_item->derivation_profile_name);
        }
...
static bool personality_content_serialize(const char * se_dir, struct personality_t * p_personality,
                                          gta_personality_name_t personality_name, UsefulBuf * hash)
{
    bool ret = false;
...
bool provider_serialize(const char * se_dir, struct devicestate_stack_item_t * p_devicestate_stack)
{
    ...
    for (size_t i = list_cnt((struct list_t *)p_devicestate_stack); i >= 1; i--) {
        ...
        while (NULL != p_personality_name_list_item) {
            ...
            QCBOREncode_AddSZStringToMap(&encode_ctx, LABEL_PERSONALITY_NAME,
                                         p_personality_name_list_item->personality_name);
            QCBOREncode_AddSZStringToMap(&encode_ctx, LABEL_APPLICATION_NAME,
                                         p_personality_name_list_item->application_name);
            QCBOREncode_AddSZStringToMap(&encode_ctx, LABEL_PERSONALITY_IDENTIFIER,
...
static bool decode_attributes(QCBORDecodeContext * p_decode_ctx, struct personality_t * p_personality,
...
static bool auth_info_list_deserialize(gta_context_handle_t h_ctx, QCBORDecodeContext * p_decode_ctx,
                                       struct auth_info_list_item_t ** p_auth_list, gta_errinfo_t * p_errinfo)
{
    bool ret = false;
...
static bool personality_content_deserialize(const char * se_dir, struct personality_t * p_personality,
                                            gta_personality_name_t personality_name,
                                            struct q_useful_buf_c * expected_hash, gta_context_handle_t h_ctx)
{
    bool ret = false;
    gta_errinfo_t errinfo;
...
static bool decode_identifiers(QCBORDecodeContext * p_decode_ctx, struct devicestate_stack_item_t * p_devicestack_item,
                               gta_context_handle_t h_ctx)
{
    bool ret = false;
...
static bool decode_personalities(const char * se_dir, QCBORDecodeContext * p_decode_ctx,
                                 struct devicestate_stack_item_t * p_devicestack_item, gta_context_handle_t h_ctx)
{
    bool ret = false;
    ...
    while (1) {
        ...
        list_append_front((struct list_t **)(&p_devicestack_item->p_personality_name_list),
                          p_personality_name_list_item);
        ...
        while (NULL != p_devicestack_item_temp) {
            p_identifier_list_item = list_find((struct list_t *)(p_devicestack_item_temp->p_identifier_list), id_name,
...
static bool decode_devicestates(const char * se_dir, QCBORDecodeContext * p_decode_ctx,
                                struct devicestate_stack_item_t ** pp_devicestate_stack, gta_context_handle_t h_ctx)
{
    bool ret = false;
...
bool provider_deserialize(const char * se_dir, struct devicestate_stack_item_t ** pp_devicestate_stack,
                          gta_context_handle_t h_ctx)
{
    bool ret = false;
    gta_errinfo_t errinfo;
    ...
    if ((NULL != file_buffer) && (0 != file_buffer_size)) {
        ...
        t_cose_result = t_cose_mac_validate(&validate_ctx, DecodeStorage,           /* COSE to validate */
                                            NULL_Q_USEFUL_BUF_C, &returned_payload, /* Payload from maced_cose */
                                            NULL);
