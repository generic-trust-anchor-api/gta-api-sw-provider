...
static bool generate_access_token (struct provider_instance_auth_token_t * p_auth_token_list_item)
{
    ...
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->target_personality_fingerprint), sizeof(gta_personality_fingerprint_t))) {
        goto err;
    }
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->type), sizeof(gta_access_descriptor_type_t))) {
        goto err;
    }
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->usage), sizeof(gta_access_token_usage_t))) {
        goto err;
    }
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->freshness), sizeof(uint32_t))) {
        goto err;
    }
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->binding_personality_fingerprint), sizeof(gta_personality_fingerprint_t))) {
        goto err;
    }
    if (!EVP_DigestUpdate(ctx, &(p_auth_token_list_item->derivation_profile), sizeof(uint32_t))) {
        goto err;
    }
