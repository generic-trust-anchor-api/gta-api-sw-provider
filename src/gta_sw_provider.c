    ...
    )
{
    ...
    /* check whether function is supported by profile */
    if (NULL == supported_profiles[p_context_params->profile].pFunction->personality_attribute_type_supported) {
        DEBUG_PRINT(("gta_personality_add_(trusted_)attribute: Profile not supported\n"));
        *p_errinfo = GTA_ERROR_PROFILE_UNSUPPORTED;
        goto err;
    }

    /* Check whether profile defines support for this attribute type */
    if (!supported_profiles[p_context_params->profile].pFunction->personality_attribute_type_supported(pers_attr_type)) {
    ...
    ))
{
    ...
    return personality_add_attribute(h_ctx, attrtype, attrname, p_attrvalue, true, p_errinfo);
}
    ...
    ))
{
    ...
    return personality_add_attribute(h_ctx, attrtype, attrname, p_attrvalue, false, p_errinfo);
}
    ...
    ))
{
    ...
    /* Check whether profile defines support for this attribute type */
    if (!supported_profiles[p_context_params->profile].pFunction->personality_attribute_type_supported(p_attribute->type)) {
        *p_errinfo = GTA_ERROR_INVALID_ATTRIBUTE;
        goto err;
    }
